...
\noindent GPT-OSS 120B serves as a \emph{negative control}: its
training data is known to exclude GSM8K, so any observed gap reflects
...
the negative control---is the first indicator that these results
...
five models---including the negative control (GPT-OSS
120B)---appear uniformly ``contaminated'' at $\Delta \approx +0.42$.
\textbf{Right:} After independent verification and pipeline repair,
the signal collapses to $\Delta \approx +0.04$--$0.11$. Two models
show mild gaps; three are statistically invariant.}
\label{fig:pre_post}
\end{figure*}
...
lose 42 percentage points when numeric values change, a far larger
effect than the 5--15 percentage-point baseline reported by
GSM-Symbolic~\cite{mirzadeh2024gsmsymbolic}. Second, the uniformity
is implausible: if the gap reflected contamination, we would expect
variation across models with different training data and sizes, yet
all five models---including GPT-OSS 120B (no GSM8K in training
data)---show $\Delta \in [+0.40, +0.47]$. This uniformity suggests
an item-level property, not a model-level one.


% ------------------------------------------------------------------
\subsection{Verification Audit: Seven Bug Classes}
\label{sec:audit}
...
annotation-chain replay. We identified seven distinct bug classes:
...
We used two complementary verification methods: (1)~multi-model
...
executing the reasoning graph with the mutated leaf values. Method~(2)
is the definitive check; method~(1) was used as an initial screening
tool.
...
from multi-step intermediates, unparseable annotation chains). The
final verified dataset contains \textbf{69 items} (59~verified-clean
+ 10~regenerated).


% ------------------------------------------------------------------
\subsection{Corrected Results (N=69, Verified)}
\label{sec:corrected}
...
\paragraph{Three of five models are invariant.}
Llama~4 Scout~17B, GPT-OSS~120B, and Llama~3.3~70B all exhibit
small, statistically non-significant accuracy gaps between original
and variant items
($\Delta_{\mathrm{iso}} \in [+0.041, +0.048]$, all $p > 0.05$).
The negative control (GPT-OSS~120B,
$\Delta_{\mathrm{iso}} = +0.048$, $p = 0.174$) confirms that the
$\tau$-isomorphic variants are not systematically harder. Notably,
Scout~17B achieved 100\% accuracy on all 50~regenerated variant
items, demonstrating perfect reasoning transfer to novel numeric
contexts.

\paragraph{Two models show mild gaps.}
Llama~3.1~8B exhibits a statistically significant but modest
accuracy gap: 95.7\% on originals versus 84.4\% on variants
($\Delta_{\mathrm{iso}} = +0.113$, bootstrap 95\%
CI: $[+0.049, +0.180]$, $p = 0.001$). This 11.3-point gap exceeds
the control's baseline by approximately 6~percentage points, but is
within the range reported by GSM-Symbolic for numeric perturbation
sensitivity (up to 15~percentage
points)~\cite{mirzadeh2024gsmsymbolic}. Qwen3~32B shows a
borderline gap ($\Delta_{\mathrm{iso}} = +0.060$,
$p = 0.044$), barely exceeding the negative control's $+0.048$.
Given that the Qwen3 effect size is small and within
the GSM-Symbolic baseline, and considering the limited sample size
($N = 45$), this gap is more consistent with perturbation sensitivity
than contamination. Whether the Llama~8B gap reflects contamination,
perturbation sensitivity, or a combination cannot be determined from
these data alone.
...
answers. \textbf{Right:} Post-verification---concentrated near zero,
with 40 of 59 items at $\delta = 0$ (perfect transfer).}
...
model. Only the uniformity across models (including a negative control)
...
fragility, not memorization. The negative control's gap
($\Delta = +0.048$) establishes the baseline; only gaps significantly
...
answer verification, ideally via a negative control model.
...
Based on our experience, we recommend: (1)~always include a negative
control model with known-clean training data; (2)~verify variant
...
Llama~3.1~8B's and Qwen3~32B's mild gaps. But $N = 69$ is limited,
...
Several caveats apply. First, $N = 69$ is smaller than ideal; 31~items
