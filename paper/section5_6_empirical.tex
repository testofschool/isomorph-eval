...
all items.
We note that Qwen3's model card recommends non-greedy decoding
(temperature 0.6) for thinking mode; our uniform greedy protocol may
understate Qwen3's optimal performance but ensures cross-model
comparability.
...
items}. The entity audit verifies name-level consistency (no leftover
original proper nouns, no substring artifacts, no duplicate variant
texts); it does not check pronoun--entity coreference agreement, which
we leave to future surface-quality improvements.
...
The final entity-clean dataset (v3, $N{=}63$) used in Table~\ref{tab:main}
