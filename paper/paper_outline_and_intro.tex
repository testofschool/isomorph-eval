...
Empirical evaluation on three real LLMs confirms that the framework
detects statistically significant contamination ($p < 10^{-4}$) in
all three tested models, with $\Delta_{\mathrm{raw}}$ from $+0.370$
to $+0.417$ on 100 stratified GSM8K items. IRT weighting amplifies
the contamination signal by 2.2--2.4$\times$.
\end{enumerate}
