...
generative engine (\S\ref{sec:engine}), and the perturbation sensitivity diagnostic
(\S\ref{sec:delta}).
...
reduces the risk of direct surface memorization. Note that web-novelty
is necessary but not sufficient for guaranteeing absence from all
training corpora, as training sets may include non-public or synthetic
data. Unlike prior rephrasing
...
\begin{multline}
  h(G_i) = \text{SHA256}\bigl(|N_i|,\, |E_i|,\,
  \text{depth}(G_i),\, \\
  [\tau(n)]_{n \in \text{topo}(G_i)},\,
  [\phi(n)]_{n \in \text{topo}(G_i)}\bigr)
\end{multline}
computed over the topological ordering of nodes. Two items can be
$\tau$-isomorphic only if
...
% ------------------------------------------------------------------
\subsection{The Perturbation Sensitivity Delta}
\label{sec:delta}

\begin{definition}[Item-Level Isomorphic Delta]
...
Under $H_0$ (no perturbation sensitivity), $\mathbb{E}[\delta_i] = 0$ because
$i \cong_\tau i'_k$ implies identical IRT parameters (Conjecture~\ref{conj:rsh}).
A positive $\delta_i$ may indicate memorization, numeric perturbation
sensitivity, or both; distinguishing these requires comparison with a
comparison model (Section~\ref{sec:results}).
\end{definition}

\begin{definition}[IRT-Weighted Isomorphic Delta]
\label{def:model_delta}
For model $M$ evaluated on benchmark $B = \{i_1, \ldots, i_N\}$:
\begin{equation}
\Delta_{\mathrm{iso}}^{\mathrm{IRT}}(M, B) = \frac{1}{N}
...
\label{eq:wald}
\end{equation}
with Bonferroni correction: item $i$ is flagged as showing DIF if
...
Based on the pattern of ($\Delta_{\mathrm{iso}}$, $p$-value,
original accuracy), we classify models into four categories:
\emph{Invariant} ($p > 0.05$, gap consistent with baseline
perturbation sensitivity);
\emph{Mild Gap} ($p < 0.05$, $\Delta < 0.15$);
\emph{Partial Memorizer} ($\Delta \geq 0.15$);
and \emph{Pure Memorizer} ($\Delta > 0.30$, original accuracy $> 85\%$).
We note that our empirical results (Section~\ref{sec:results}) found
no models in the latter two categories on verified data; the
classification remains theoretical pending evaluation on a larger,
fully verified dataset.
